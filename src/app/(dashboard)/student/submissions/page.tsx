...
export const metadata = {
  title: "My Submissions | Python Learning Platform",
};
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          View your past code submissions and grading results by week.
        </p>
      </div>
      ...
      {!hasAnySubmissions ? (
        <Card className="bg-background border-border">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground max-w-sm">You haven&apos;t submitted code for any problems yet.</p>
          </CardContent>
        </Card>
      ) : (
                            ...
                            return (
                              <div key={problem.id} className="border border-border rounded-md bg-card overflow-hidden">
                                ...
                                <div className="divide-y divide-border">
                                  {problem.submissions.map((sub, index) => {
                                    const config = getStatusConfig(sub.status);
                                    const Icon = config.icon;
                                    const attemptNum = problem.submissions.length - index;
                                    return (
                                      <div key={sub.id} className="px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-muted/10 transition-colors">
                                        <div className="flex flex-col">
                                          <span className="text-sm font-medium text-foreground">Attempt {attemptNum}</span>
                                        ...
                                        <div className="flex items-center gap-4 text-sm">
                                          ...
                                          <span className="text-muted-foreground text-xs w-16 text-right">{sub.passedTests}/{sub.totalTests} tests</span>
                                          <Link href={`/student/submissions/${sub.id}`} className="text-xs font-medium text-[#00A8E8] hover:underline ml-2">
                                            View &rarr;
                                          </Link>
                                        </div>
                                      </div>
