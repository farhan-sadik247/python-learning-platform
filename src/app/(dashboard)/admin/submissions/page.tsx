...
export const metadata = {
  title: "All Submissions | Admin Dashboard",
};
...
export default async function AdminSubmissionsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Platform Submissions</h1>
        <p className="text-muted-foreground mt-2">
          Global view of all student code submissions, organized by course and week.
        </p>
      </div>
      ...
      {!hasAnySubmissions ? (
        <Card className="bg-background border-border">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground max-w-sm">No students have submitted code on the platform yet.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            return (
              <div key={course.id} className="space-y-4">
                <div className="flex items-center gap-2 border-b border-border pb-2">
                  ...
                  <span className="text-sm text-muted-foreground ml-2">by {course.teacher.name}</span>
                </div>
                                    ...
                                    return (
                                      <div key={sub.id} className="px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-muted/10 transition-colors">
                                        ...
                                        <div className="flex items-center gap-4 text-sm">
                                          ...
                                          <Link href={`/admin/submissions/${sub.id}`} className="text-xs font-medium text-[#00A8E8] hover:underline ml-2">
                                            View &rarr;
                                          </Link>
                                        </div>
                                      </div>
